                    ...
                    <div class="col-md-9">
                        @foreach($blogPosts as $blog)
                            <div class="blog-post  wow fadeInUp">
                                <a href="{{ route('post.details',$blog->id) }}"><img class="img-responsive" src="{{ asset($blog->post_image) }}" alt="" style="height: 100%;width: 100%;"></a>

                                <h1><a href="{{ route('post.details',$blog->id) }}"> @if(session()->get('language') == 'english') {{ $blog->post_title_english }} @else {{ $blog->post_title_bangla }} @endif</a></h1>

                                <span class="date-time"> {{ Carbon\Carbon::parse($blog->created_at)->diffForHumans()  }}</span>

                                <p>@if(session()->get('language') == 'english') {!! Str::limit($blog->post_details_english, 200 )  !!} @else {!! Str::limit($blog->post_details_bangla, 200 )  !!} @endif</p>
